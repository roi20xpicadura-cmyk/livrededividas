    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-page)' }}>
      <div className="p-7 flex flex-col gap-5 max-w-[1400px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Total Receitas', value: totals.inc, color: '#16a34a', bg: '#dcfce7', icon: TrendingUp, sub: `${totals.incCount} receitas`, valColor: '#16a34a' },
            { label: 'Total Despesas', value: totals.exp, color: '#dc2626', bg: '#fee2e2', icon: TrendingDown, sub: `${totals.expCount} despesas`, valColor: '#dc2626' },
            { label: 'Saldo do Período', value: totals.net, color: '#2563eb', bg: '#eff6ff', icon: Scale, sub: 'Resultado líquido', valColor: totals.net >= 0 ? '#16a34a' : '#dc2626' },
          ].map((s, i) => (
            <div key={i} className="flex items-center gap-3.5 bg-card border-[1.5px] border-border rounded-xl p-4 hover:border-[#86efac] transition-colors duration-200">
              <div className="w-10 h-10 rounded-[10px] flex items-center justify-center shrink-0" style={{ background: s.bg }}>
              ...
              <div className="min-w-0">
                ...
                <p className="text-[20px] font-black truncate" style={{ color: s.valColor }}>{formatCurrency(s.value)}</p>
                <p className="text-[11px] text-muted-foreground">{s.sub}</p>
              </div>
            </div>
          ))}
          {/* Card 4: count */}
          <div className="flex items-center gap-3.5 bg-card border-[1.5px] border-border rounded-xl p-4 hover:border-[#86efac] transition-colors duration-200">
            <div className="w-10 h-10 rounded-[10px] flex items-center justify-center shrink-0 bg-muted/30">
              <Receipt className="w-[18px] h-[18px] text-muted-foreground" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-wide">Lançamentos</p>
              <p className="text-[20px] font-black text-foreground">{txs.length}</p>
              {limits.transactions_per_month !== Infinity ? (
                <div>
                  <p className="text-[11px] text-muted-foreground">{txs.length}/{limits.transactions_per_month} este mês</p>
                ...
                <p className="text-[11px] text-[#16a34a] font-semibold">Ilimitados ✓</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div ref={formRef} className="bg-card border-[1.5px] border-border rounded-2xl overflow-hidden">
          {/* Header */}
          <div className={`flex items-center justify-between px-5 py-4 border-b border-border/50 transition-colors duration-200 ${type === 'income' ? 'bg-secondary' : 'bg-[#fef2f2]'}`}>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowImport(true)} className="ml-2 flex items-center gap-1.5 px-3 py-1.5 text-xs text-muted-foreground border-[1.5px] border-border rounded-[7px] hover:border-[#16a34a] hover:text-[#16a34a] transition-colors">
            ...
            <div className="flex gap-2">
              {(['income', 'expense'] as const).map(t => (
                <button key={t} onClick={() => { setType(t); setCat(''); }}
                  className={`flex items-center gap-1.5 px-4 py-[7px] rounded-lg text-[13px] font-extrabold border-2 transition-all duration-200 cursor-pointer ${
                    type === t
          ...
          {/* Form body */}
          <div className="p-5" onKeyDown={e => { if (e.key === 'Enter' && !submitting) handleAdd(); }}>
            {/* Row 1 */}
                ...
                <div className="flex gap-1">
                  {(['personal', 'business'] as const).map(o => (
                    <button key={o} onClick={() => setOrigin(o)}
                      className={`flex-1 px-2 py-2 rounded-lg text-[12px] font-bold border-[1.5px] transition-all duration-200 ${
                        origin === o
              ...
              <div>
                ...
                <div className="flex items-center gap-2 h-[42px]">
                  <button onClick={() => setIsRecurring(!isRecurring)}
                    className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${isRecurring ? 'bg-[#16a34a]' : 'bg-[#e2e8f0]'}`}>
            ...
            <div className="flex items-center justify-between mt-1">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleAdd} disabled={submitting}
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-[9px] text-[13px] font-extrabold text-white transition-all duration-200 active:scale-[0.97] ${
        ...
        {/* ── 3. FILTER BAR ── */}
        <div className="bg-card border-[1.5px] border-border rounded-xl p-3 px-4 flex items-center gap-2.5 flex-wrap">
          <div className="flex items-center gap-2 flex-wrap flex-1">
            {([['all', 'Todos'], ['income', 'Receitas ↑'], ['expense', 'Despesas ↓']] as const).map(([v, l]) => (
              <button key={v} onClick={() => { setFilterType(v); setPage(1); }}
                className={`px-3.5 py-1.5 rounded-full text-[12px] font-bold border-[1.5px] transition-all duration-200 ${
                  filterType === v
            ...
            <div className="w-px h-5 bg-muted/30" />
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Buscar descrição..."
                className="w-[220px] h-9 pl-8 pr-8 text-[13px] rounded-[9px] border-[1.5px] border-border bg-background focus:bg-card focus:border-[#16a34a] focus:outline-none transition-colors" />
              {search && (
            ...
            <div className="relative">
              <button onClick={() => setShowSortDropdown(!showSortDropdown)}
                className="flex items-center gap-1 px-3 py-2 text-[12px] font-semibold text-muted-foreground hover:text-[#16a34a] transition-colors">
            ...
            {hasActiveFilters && (
              <button onClick={clearFilters} className="flex items-center gap-1 text-[11px] font-semibold text-[#dc2626] hover:underline">
            ...
            <AnimatePresence>
              {showCatBreakdown && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  className="bg-card border-[1.5px] border-border rounded-xl p-5 overflow-hidden">
                  ...
                  {catBreakdown.slice(0, 8).map((c, i) => {
                    ...
                    return (
                      <div key={c.name} className="flex items-center gap-2.5 mb-2.5">
                        <span className="text-[12px] font-semibold text-foreground min-w-[120px] truncate">{c.name}</span>
                        ...
                        <span className={`text-[12px] font-bold min-w-[80px] text-right ${c.type === 'income' ? 'text-[#16a34a]' : 'text-[#dc2626]'}`}>
                          {formatCurrency(c.amount)}
                        </span>
                        <span className="text-[11px] text-muted-foreground min-w-[40px] text-right">
                          {((c.amount / (c.type === 'income' ? totals.inc : totals.exp || 1)) * 100).toFixed(0)}%
                        </span>
                      </div>
                    );
                  })}
        ...
        {filtered.length === 0 ? (
          /* Empty state */
          <div className="bg-card border-[1.5px] border-border rounded-2xl flex flex-col items-center py-14 px-6 text-center gap-3.5">
            <div className="w-[88px] h-[88px] rounded-full bg-secondary flex items-center justify-center">
              <ReceiptText className="w-10 h-10 text-[#86efac]" />
            </div>
            <p className="text-[17px] font-extrabold text-foreground">Nenhum lançamento ainda</p>
            ...
            <div className="flex gap-2.5 mt-2">
              <button onClick={() => scrollToForm('income')}
                className="px-5 py-2.5 rounded-[9px] text-[13px] font-extrabold bg-[#dcfce7] text-[#166534] border-[1.5px] border-[#d4edda] hover:brightness-95 transition-all">
                + Adicionar Receita
              </button>
              <button onClick={() => scrollToForm('expense')}
                className="px-5 py-2.5 rounded-[9px] text-[13px] font-extrabold bg-[#fee2e2] text-[#991b1b] border-[1.5px] border-[#fecaca] hover:brightness-95 transition-all">
                + Adicionar Despesa
              </button>
            </div>
          </div>
          ...
          <div className="bg-card border-[1.5px] border-border rounded-[14px] overflow-hidden">
            ...
            <AnimatePresence>
              {selected.size > 0 && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                  className="bg-[#14532d] px-5 py-2.5 flex items-center gap-3 text-white overflow-hidden">
                  <span className="text-[13px] font-semibold">{selected.size} selecionado(s)</span>
                  <button onClick={handleBulkDelete}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold bg-white/15 border border-white/30 hover:bg-[#dc2626] transition-colors">
                    <Trash2 className="w-3 h-3" /> Excluir selecionados
                  </button>
                  <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-semibold bg-white/15 border border-white/30 hover:bg-white/25 transition-colors">
                    <Download className="w-3 h-3" /> Exportar
                  </button>
                  <button onClick={() => setSelected(new Set())} className="ml-auto text-[12px] hover:underline">Cancelar</button>
                </motion.div>
              )}
            </AnimatePresence>
                ...
                <tbody>
                  <AnimatePresence>
                    {paginated.map((tx, i) => {
                      ...
                      return (
                        <motion.tr key={tx.id}
                          initial={{ opacity: 0, y: -16, backgroundColor: '#f0fdf4' }}
                          animate={{ opacity: 1, y: 0, backgroundColor: isSelected ? '#f0fdf4' : isDeleting ? '#fef2f2' : '#ffffff' }}
            ...
            {/* Footer / pagination */}
            <div className="flex items-center justify-between px-5 py-3 border-t-[1.5px] border-border/50 bg-background text-[12px]">
        ...
        <AnimatePresence>
          {showImport && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => { setShowImport(false); setCsvData(null); }}>
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                onClick={e => e.stopPropagation()}
                className="bg-card rounded-2xl max-w-[600px] w-full max-h-[80vh] overflow-y-auto shadow-xl">
                ...
                <div className="p-5">
                  {!csvData ? (
                    <div
                      className="border-2 border-dashed border-[#d4edda] rounded-xl p-10 text-center bg-background cursor-pointer hover:border-[#16a34a] transition-colors"
