...
export default function OnboardingChat({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    if (firstName) {
      ...
      addMessage({
        role: 'assistant',
        content: `Olá, ${firstName}! 👋\n\nEu sou a **FinDash IA**, sua assistente financeira pessoal. Vou te ajudar a configurar tudo em menos de 1 minuto.\n\nComo você quer usar o FinDash Pro?`,
      ...
      addMessage({
        role: 'assistant',
        content: `Olá! 👋\n\nEu sou a **FinDash IA**, sua assistente financeira pessoal.\n\nAntes de tudo, como posso te chamar?`,
        inputType: 'text',
      }, 800);
      setCurrentStep('name');
    }
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid var(--color-border-weak)' }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--color-text-strong)' }}>FinDash IA</p>
              ...
              <div className={`max-w-[85%] ${msg.role === 'user' ? '' : ''}`}>
                {msg.role === 'assistant' && (
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-subtle)' }}>FinDash IA</span>
                  </div>
                )}
