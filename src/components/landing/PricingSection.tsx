...
export default function PricingSection() {
  ...
  const plans = [
    {
      ...
      features: [
        ...
        { label: 'Score de saúde financeira', included: true },
        ...
        { label: 'Lançamentos ilimitados', included: false },
        { label: 'Exportação', included: false },
    ...
    },
    {
      ...
      features: [
        ...
        { label: 'Relatório mensal por e-mail', included: true },
        { label: 'Orçamento inteligente', included: true },
  ...
  return (
    <section id="precos" className="py-20 md:py-28 px-4 bg-[#f8fafc]">
      <div className="max-w-[1100px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-4">
          <h2 className="text-[clamp(28px,4vw,48px)] font-[900] text-[#0f172a] tracking-[-2px]">Preço justo. Valor real.</h2>
          <p className="text-[18px] text-[#64748b] mt-2">Comece grátis. Faça upgrade quando precisar.</p>
        </motion.div>

        {/* Toggle */}
        <div className="flex items-center justify-center gap-3 mb-12">
          <span className={`text-[14px] font-semibold transition-colors ${!annual ? 'text-[#0f172a]' : 'text-[#94a3b8]'}`}>Mensal</span>
          <button
            onClick={() => setAnnual(!annual)}
            className={`w-12 h-6 rounded-full transition-colors duration-200 relative ${annual ? 'bg-[#16a34a]' : 'bg-[#e2e8f0]'}`}
          >
          ...
          <span className={`text-[14px] font-semibold transition-colors ${annual ? 'text-[#0f172a]' : 'text-[#94a3b8]'}`}>Anual</span>
          {annual && <span className="text-[11px] font-bold text-[#854d0e] bg-[#fef9c3] px-2 py-0.5 rounded-full">Economize 20%</span>}
        </div>
              ...
              <motion.div
                ...
                className={`relative rounded-[20px] p-8 ${
                  p.dark
                    ...
                    : p.featured
                    ? 'bg-white border-2 border-[#16a34a] md:scale-[1.04]'
                    : 'bg-white border border-[#e2e8f0]'
                }`}
                style={p.featured ? { boxShadow: '0 20px 60px rgba(22,163,74,0.15)' } : p.dark ? {} : {}}
              >
                {p.featured && (
                  <span
                    className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-[#16a34a] text-white text-[12px] font-bold whitespace-nowrap"
                ...
                <div className={`text-[14px] font-semibold ${p.dark ? 'text-white/70' : 'text-[#64748b]'}`}>{p.name}</div>
                ...
                <div className="mt-2 mb-1">
                  <AnimatePresence mode="wait">
                    <motion.span
                      ...
                      className={`text-[48px] font-[900] ${p.dark ? 'text-white' : p.featured ? 'text-[#0f172a]' : 'text-[#0f172a]'}`}
                    >
                      {price === 0 ? 'Grátis' : `R$ ${price}`}
                    </motion.span>
                  </AnimatePresence>
                  {price > 0 && <span className={`text-[14px] ${p.dark ? 'text-white/50' : 'text-[#94a3b8]'}`}>/mês</span>}
                </div>
                {annual && price > 0 && (
                  <div className="text-[12px] text-[#16a34a] font-semibold bg-[#f0fdf4] inline-block px-2 py-0.5 rounded-full mb-1">
                    Economize R$ {(p.price - p.priceAnnual) * 12}/ano
                  </div>
                )}
                <p className={`text-[13px] mb-6 ${p.dark ? 'text-white/50' : 'text-[#94a3b8]'}`}>{p.sub}</p>

                <div className={`border-t mb-6 ${p.dark ? 'border-white/10' : 'border-[#f1f5f9]'}`} />
                ...
                <Link
                  to="/register"
                  className={`block w-full py-3.5 rounded-[12px] text-[15px] font-[800] text-center transition-all duration-200 ${
                    p.dark
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-8 bg-[#f0fdf4] border border-[#bbf7d0] rounded-2xl p-5 md:px-8 flex flex-col md:flex-row items-center gap-4"
        >
          <Shield className="w-8 h-8 text-[#16a34a] flex-shrink-0" />
          <div>
            <div className="text-[16px] font-bold text-[#0f172a]">Garantia de 7 dias</div>
            <div className="text-[14px] text-[#64748b]">Se não ficar satisfeito, devolvemos 100% do valor. Sem perguntas.</div>
          </div>
        </motion.div>
      </div>
    </section>
