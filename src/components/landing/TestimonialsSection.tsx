...
const testimonials = [
  { quote: 'Finalmente encontrei algo que junta minha loja Hotmart com minhas finanças pessoais. Importei R$ 23.000 em vendas e já apareceu tudo organizado. Nunca mais vou usar planilha.', highlight: 'R$ 23.000 em vendas', name: 'Ana Beatriz M.', role: 'Produtora de Conteúdo, SP', gradient: 'from-[#7C3AED] to-[#22c55e]' },
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4">
          {testimonials.map((t, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5, ease }} className="break-inside-avoid bg-white border border-[#e2e8f0] rounded-2xl p-6 hover:border-[#C4B5FD] transition-colors duration-200">
              ...
              <p className="text-[15px] text-[#0f172a] leading-[1.7] mb-4">
                "{t.quote.split(t.highlight).map((part, pi) => (
                  <span key={pi}>{part}{pi < t.quote.split(t.highlight).length - 1 && <strong className="text-[#7C3AED] font-bold">{t.highlight}</strong>}</span>
                ))}"
              </p>
              <div className="flex items-center gap-3">
                ...
                <span className="ml-auto text-[11px] text-[#7C3AED] font-medium">✓ Verificado</span>
              </div>
            </motion.div>
          ))}
        </div>
